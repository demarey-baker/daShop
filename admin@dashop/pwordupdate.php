	...
	if(isset($_POST['pwordUpdate'])){
		...
		if($entry1&&$entry8){
			...
			if($daShopConn){
				...
				$uID=$_SESSION['adminemail'];		

				$curUser="SELECT Password from admin WHERE AdminEmail='$uID'";
				...
				if(mysqli_num_rows($testCurUser)>0){
					while($row=mysqli_fetch_assoc($testCurUser)){
						if($uCurPword==$row['Password']){
							$pword_update="UPDATE admin set Password='$uNewPword' where AdminEmail='$uID'";
							$run_pword_update=mysqli_query($daShopConn,$pword_update);

							$other_activity_log="INSERT into activitylogs(User_ID,OtherActivity,TimeDate) values('Admin','Admin changed password','".date("l, F j, Y, g:i A")."')";
							$run_other_activity_log=@mysqli_query($daShopConn,$other_activity_log);	

							if($run_pword_update){
								if (headers_sent()){
									echo '<script>alert("Password updated");</script>';
								    die('<script type="text/javascript">window.location.href="' . "admin@settings.php" . '";</script>');
								}
								else{
