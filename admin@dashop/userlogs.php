	...
	if($daShopConn){
		...
		if(@mysqli_num_rows($run_get_logs)>0){
			...
			if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
				...
				$results = mysqli_query($daShopConn,"SELECT COUNT(*) FROM userlogs");
				...
				$results = mysqli_query($daShopConn,"SELECT * from userlogs ORDER BY Log_ID DESC LIMIT $page_position, $item_per_page");
