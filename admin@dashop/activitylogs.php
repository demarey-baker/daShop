<?php
	...
	if($daShopConn){
		$get_logs="SELECT * from activitylogs";
		...
		if(@mysqli_num_rows($run_get_logs)>0){
			...
			if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
				...
				$results = mysqli_query($daShopConn,"SELECT COUNT(*) FROM activitylogs");
				...
				$results = mysqli_query($daShopConn,"SELECT * from activitylogs ORDER BY Log_ID DESC LIMIT $page_position, $item_per_page");
				...
				echo '<th>Log ID</th>';
				echo '<th>User ID</th>';
				echo '<th>Purchase activity</th>';
				echo '<th>Sales activity</th>';
				...
				while($rows=mysqli_fetch_assoc($results)){
					...
					echo '<td>'.$rows['PurchaseActivity'].'</td>';
					echo '<td>'.$rows['SalesActivity'].'</td>';
					echo '<td>'.$rows['OtherActivity'].'</td>';
					echo '<td>'.$rows['TimeDate'].'</td>';
					echo '</tr>';
				}//end while
				echo '</table>';
				echo '</ul>';
