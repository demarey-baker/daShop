	...
	if($daShopConn){
		...
		if(@mysqli_num_rows($run_get_users)>0){
			...
			if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
				...
				$results = mysqli_query($daShopConn,"SELECT COUNT(*) FROM users where User_ID=".$uID."");
				...
				$results = mysqli_query($daShopConn,"SELECT * from users where User_ID=".$uID." ORDER BY User_ID DESC LIMIT $page_position, $item_per_page");
