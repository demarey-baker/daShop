		...
		if($selectAutoItems){
			if(@mysqli_num_rows($selectAutoItems)>0){
				...
				if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
					...
					$results = mysqli_query($daShopConn,"SELECT AutoNum_ID, ItemName, Price, Product_Img, ProductOverlay, DateAdded, ItemStatus, ProductSellerID from automobiles where ItemName like '".$q."%' or ItemName like '%".$q."' or ItemName like '%".$q."%' ORDER BY AutoNum_ID ASC LIMIT $page_position, $item_per_page");
					
					//Display records fetched from database.

					$_SESSION['search_number']=(@mysqli_num_rows($results)>0)?@mysqli_num_rows($results):0;
					$_SESSION['search_number'].=" item were found for ".$q;
			...
			echo '<script>alert("No item was found");</script>';
		}
